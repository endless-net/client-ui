...
enum ClientPage { connection, network, settings, support }
...
class ClientSessionPanel extends StatelessWidget {
  ...
  Widget _section(BuildContext context, Widget panel) {
  ...
  ClientPage _page(Widget panel) => switch (panel) {
    ClientConnectionPanel() ||
    ClientCreateProfilePanel() ||
    ClientEnrollmentPanel() ||
    ClientRecoveryPanel() ||
    ClientRuntimeOperationsPanel() => ClientPage.connection,
    ClientProfilesPanel() ||
    ClientNetworksPanel() ||
    ClientPeersPanel() ||
    ClientResourcesPanel() ||
    ClientExitPanel() => ClientPage.network,
    ClientSupportPanel() ||
    ClientDiagnosticsPanel() ||
    ClientIdentityPanel() => ClientPage.support,
  ...
  int _order(Widget panel) => switch (panel) {
    ...
    ClientEnrollmentPanel() || ClientPeersPanel() => 2,
    _ => 10,
  };

  Widget _layout(BuildContext context, List<Widget> panels) => IndexedStack(
    index: page.index,
  ...
  @override
  Widget build(BuildContext context) => _layout(context, <Widget>[
    ?notifications,
    ClientSupportPanel(
      locale: locale,
      state: session.state,
          ...
          return session.submitPrivilegedViaLookup(
            ...
            (mutation) {
              if (identityEpoch !=
                  ...
                  profileEpoch !=
                      session.state.domainEpoch(api.Domain.DOMAIN_PROFILES)) {
        ...
        return session.submit(
          ...
          (commands, mutation) {
            if (identityEpoch !=
                ...
                profileEpoch !=
                    session.state.domainEpoch(api.Domain.DOMAIN_PROFILES)) {
              throw StateError('Identity changed before trust submission');
            }
