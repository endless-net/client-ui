...
heading semantics и доступность 15 начальных разделов при 360x640/200%.
Реальная навигация screen reader и полный набор загруженных состояний ещё
не проверены; это source-изменение для UF-14, не platform acceptance.

Проверка изоляции форм (2026-09-14): существующие ключи enrollment и создания
...
Текущий source gate по всем UF на `853c42d`:
...
операции не меняет новый экран и не разблокирует новый запрос. Уже отправленная
операция не отменяется и не повторяется этим UI guard.
...
результат в новом контексте и не разблокирует новый запрос. Уже отправленное
намерение остаётся в session journal; панель не отменяет и не повторяет RPC.

Recovery panel связывает незавершённые lookup/browser/export/ack с конкретным
controller lifetime, а не только cacheEpoch. При замене контроллера результаты,
notice и busy очищаются; позднее завершение старого действия не запускает новый
adapter и не изменяет состояние нового запроса. Четыре unit/widget сценария
проверяют замену на другой ready controller с тем же epoch.

Desktop shell при 200% текста: Reconnect/Quit находятся в строке с переносом,
а язык, статус и сообщения — в ограниченном прокручиваемом блоке над session
panel. Это устраняет уход Quit за правый край и вертикальное переполнение длинных
RU ошибок. Диалог неподтверждённого выхода прокручивает текст отдельно от действий.
RU/EN widget checks включают минимальный desktop-размер 620×460; OS accessibility
acceptance остаётся отдельной от этих layout/keyboard проверок.
...
пользователь может нажать «Восстановить значок в трее» в открытом окне. Native
`prepareRegistration` сбрасывает признак потери перед `setIcon`, затем UI заново
создаёт меню и проверяет host. Новый broadcast во время регистрации сохраняет
неготовность; повторные нажатия и закрытие окна на время регистрации блокируются.
Успешное восстановление само не скрывает окно и не меняет runtime intent.
Перед повторной попыткой вызывается `trayManager.destroy`: закреплённый
`tray_manager 0.5.3` иначе использует `NIM_MODIFY` вместо `NIM_ADD`. Ошибка удаления
